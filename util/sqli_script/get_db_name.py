...
# Setting row_num here equal to 0 is what should normally be done if testing the script out.
# since when row_num is set to 0 we'll be extracting the first database, which is something we know in mysql (information_schema).
# that would be to verify we are extracting the database name correctly. In this case I already knew the name of the second database (and it was shorter).
row_num = 1
max_ascii = 127
min_ascii = 0
ascii_guess = 95
