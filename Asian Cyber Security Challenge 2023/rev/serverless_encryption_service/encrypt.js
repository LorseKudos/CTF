...
function encrypt(message, password) {
  var g = [
    ...
    j = Math.floor(Math.random() * 10), // 0～9の整数:6
    k = Math.floor(Math.random() * 10), // 0～9の整数:3
    l = g[j], // 0x96e2cefe4c1441bec265963da4d10ceb46b7d814d5bc15cc44f17886a09390999b8635c8ffc7a943865ac67f9043f21ca8d5e4b4362c34e150a40af49b8a1699n
    o = h[k], // 0xf79dd7feae09ae73f55ea8aa40c49a7bc022c754db41f56466698881f265507144089af47d02665d31bba99b89e2f70dbafeba5e42bdac6ef7c2f22efa680a67n
    r = l * o,
    s = Math.floor(Math.random() * 5), // 0～4の整数:3
    t = Math.pow(2, Math.pow(2, s)) + 1; // 2^8 + 1

  function bytes_to_long(message) {
    var B = new TextEncoder().encode(message);
    let C = 0x0n;
  ...
  var encoded_message = bytes_to_long(message);

  function pow(A, B, C) {
    if (B === 0n) return 0x1n;
    return B % 2n === 0n ? pow((A * A) % C, B / 2n, C) : (A * pow(A, B - 1n, C)) % C;
  }
  var x = pow(encoded_message, t, r);
  let y = [];
  while (x > 0) {
